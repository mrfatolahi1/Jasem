...
class TimeLogStore:
    ...
    def load(self):
        """Return all stored time entries in file order."""
        return [
            TimeEntry(date=cells[0], time_text=cells[1], work=cells[2], tag=cells[3])
    ...
    def save(self, entries):
        ...
        rows = [
            [entry.date, sanitize(entry.time_text), sanitize(entry.work), sanitize(entry.tag) or "work"]
